...
module.exports = Handlebones.ModelView.extend({
  ...
  context: function () {
    ...
    const remainingCards = ctx.remaining;
    let iconClass;
    if (remainingCards % 3 === 0) {
      iconClass = "icon-cat";
    } else if (remainingCards % 3 === 1) {
      iconClass = "icon-ram";
    } else {
      iconClass = "icon-bird";
    }
  ...
  initialize: function (options) {
    ...
    function onFail(err) {
      ...
      if (!Utils.cookiesEnabled()) {
        ...
        alert(
          "Sorry, you must be registered to vote. Please sign in or contact the conversation owner."
        );
      } else {
        alert(
    ...
    this.participantAgreed = function (e) {
      ...
      serverClient
        .agree(tid, starred, this.wipVote.high_priority)
    ...
    this.participantDisagreed = function () {
      ...
      this.onButtonClicked();
      serverClient
        .disagree(tid, starred, this.wipVote.high_priority)
        .then(onVote.bind(this), onFail.bind(this));
    };
    this.participantPassed = function () {
      ...
      this.onButtonClicked();
      serverClient
        .pass(tid, starred, this.wipVote.high_priority)
        .then(onVote.bind(this), onFail.bind(this));
    };
    ...
    this.onAuthSuccess = function () {
      ...
      if (this.wipVote.vote === -1) {
        serverClient
          .agree(tid, starred, high_priority)
          .then(reloadPage, onFailAfterAuth);
      } else if (this.wipVote.vote === 0) {
        serverClient
          .pass(tid, starred, high_priority)
          .then(reloadPage, onFailAfterAuth);
      } else if (this.wipVote.vote === 1) {
        serverClient
          .disagree(tid, starred, high_priority)
          .then(reloadPage, onFailAfterAuth);
      } else {
        alert(3);
      }
